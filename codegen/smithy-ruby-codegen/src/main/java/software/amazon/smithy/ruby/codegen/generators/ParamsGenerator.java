    ...
    private static class MemberBuilder extends ShapeVisitor.Default<Void> {
        ...
        MemberBuilder(
            ...
            if (memberShape.hasTrait(DefaultTrait.class) && memberShape.hasTrait(RequiredTrait.class)) {
                ...
                this.defaultValue = Optional.of(targetShape.accept(new DefaultValueRetriever(memberShape)));
            } else {
        ...
        protected Void getDefault(Shape shape) {
            if (defaultValue.isPresent()) {
                writer.write("$Lparams.key?($L) ? $L : $L", memberSetter, rubySymbol, input,
                        defaultValue.get());
            } else {
        ...
        private void defaultComplex(Shape shape) {
            if (defaultValue.isPresent()) {
                if (checkRequired) {
                    writer.write("$1Lparams.key?($2L) ? $3L.build($4L, context: $5L) : $6L",
                            memberSetter, rubySymbol, symbolProvider.toSymbol(shape).getName(), input, context,
                            defaultValue.get());
                } else {
                    writer.write("$1L(params.key?($2L) ? $3L.build($4L, context: $5L) : $6L)",
                            memberSetter, rubySymbol, symbolProvider.toSymbol(shape).getName(), input, context,
                            defaultValue.get());
                }
                return;
