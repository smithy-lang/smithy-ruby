    ...
    module Plugins
      describe RPCv2CBOR do
        ...
        it 'does not add the handlers and stubber if :protocol is not smithy-rpc-v2-cbor' do
          ...
          expect(client.handlers).not_to include(Client::RPCv2CBOR::BuildHandler)
          expect(client.handlers).not_to include(Client::RPCv2CBOR::ParseHandler)
          expect(client.config.stubber).not_to eq(Client::RPCv2CBOR::Stubber)
        end

        it 'adds the handler and stubber if :protocol is smithy-rpc-v2-cbor' do
          expect(client.handlers).to include(Client::RPCv2CBOR::BuildHandler)
          expect(client.handlers).to include(Client::RPCv2CBOR::ParseHandler)
          expect(client.config.stubber).to eq(Client::RPCv2CBOR::Stubber)
        end

        context 'multi protocol' do
          let(:some_other_protocol) do
            Class.new(Plugin) do
              option(:protocol, default: 'some-other-protocol')
            end
          end
        ...
        context 'no stub responses' do
          before { client_class.remove_plugin(StubResponses) }

          it 'does not add the stubber if the client does not respond to :stubber' do
            expect(client.config).not_to respond_to(:stubber)
          end
        end
      end
    end
