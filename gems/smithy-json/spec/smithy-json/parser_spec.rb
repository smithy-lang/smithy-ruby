  ...
  module Json
    describe Parser do
      ...
      it 'returns an empty hash when given bytes are empty' do
        subject = described_class.new(Schema::Shapes::Prelude::String)
      ...
      context 'structures' do
        ...
        it 'parses structures' do
          ...
          expect(subject.parse(bytes).to_h).to eq(expected)
        end
        ...
        it 'builds and parses structures with jsonName' do
          ...
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(string: 'string')
        end
      end
      ...
      context 'unions' do
        it 'parses unions' do
          ...
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(union: { string: 'string' })
        end

        it 'parses unit members' do
          data = { 'union' => { 'unit' => {} } }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(union: { unit: {} })
        end

        it 'parses nil unions' do
          data = { union: nil }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq({})
        end

        it 'parses unknown members' do
          data = { union: { 'someThing' => 'someValue' } }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(union: { unknown: { 'someThing' => 'someValue' } })
        end
      ...
      context 'lists' do
        it 'parses lists' do
          data = { 'list' => ['string'] }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(list: ['string'])
        end

        it 'parses lists with nil values' do
          data = { 'list' => [nil] }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(list: [])
        end
        ...
        it 'parses sparse lists' do
          ...
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(list: [nil, 'string', nil])
        end
      end
      ...
      context 'maps' do
        it 'parses maps' do
          data = { 'map' => { 'key' => 'value' } }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(map: { 'key' => 'value' })
        end

        it 'parses maps with nil values' do
          data = { 'map' => { 'key' => nil } }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(map: {})
        end
        ...
        it 'parses sparse maps' do
          ...
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(map: { 'key' => nil, 'anotherKey' => 'value' })
        end
      end
      ...
      context 'floats' do
        it 'parses infinity' do
          data = { 'float' => 'Infinity' }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(float: Float::INFINITY)
        end
        ...
        it 'parses NaN' do
          data = { 'float' => 'NaN' }
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(float: Float::NAN)
        end
      end
      ...
      context 'timestamps' do
        ...
        it 'parses epoch seconds' do
          ...
          bytes = Json.dump(data)
          expect(subject.parse(bytes).to_h).to eq(timestamp: time)
        end
        ...
        it 'handles unrecognized timestamp formats' do
          ...
          bytes = Json.dump(data)
          expect { subject.parse(bytes) }.to raise_error(/unhandled timestamp format/)
        end
      end
    end
  end
